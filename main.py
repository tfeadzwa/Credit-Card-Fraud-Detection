...
if st.sidebar.checkbox('Show the initial data set'):
    ...
    st.write("Analyzing and visualizing the dataset to gain insights, discover patterns, and understand the underlying structure of the data")
    st.subheader("Reading and understanding the dataset")

    st.write('Initial data set: \n', df)
    st.write("<p style='margin-bottom: -1rem; font-weight:bold;'>Data decription:</p>", unsafe_allow_html=True)
    st.write('Computes summary statistics such as mean, median, standard deviation, minimum, maximum, and quartiles for numerical features to understand their central tendency, spread, and distribution...\n', df.describe())
    st.write('Shape of the dataframe: ',df.shape)
    st.text('The dataset consists of 284,807 rows and 31 columns.\nThere is no zero value in the data.')

    st.header("Handling missing and outlier values")
    st.subheader("Handling missing values in columns")
